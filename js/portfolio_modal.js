...
$(document).on('click', '.portfolio-thumb', function () {
    ...
    var mediaHtml = isVideo
        ? `<video src="${src}" style="width:80vw;max-height:85vh;display:block;box-shadow:0 8px 40px rgba(0,0,0,0.7);border-radius:18px;" autoplay loop muted playsinline controls></video>`
        : `<img src="${src}" alt="${alt}" style="max-width:90vw;max-height:90vh;box-shadow:0 8px 40px rgba(0,0,0,0.7);border-radius:18px;">`;
    var modalHtml = `
